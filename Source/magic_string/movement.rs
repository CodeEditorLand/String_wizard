...
impl<'text> MagicString<'text> {
	...
	pub fn relocate(&mut self, start:usize, end:usize, to:usize) -> &mut Self {
		...
		// `new_right_idx` is `None` means that the `to` index is at the end of the
		// string. Moving chunks which contain the last chunk to the end is
		// meaningless.
      ...
      .unwrap_or(Some(self.last_chunk_idx));
		...
		// Adjust next/prev pointers, this remove the [start, end] range from the old
		// position

...
pub struct IndentOptions<'a, 'b> {
	/// MagicString will guess the `indentor` from lines of the source if passed
	/// `None`.
	pub indentor:Option<&'a str>,

	/// The reason I use `[u32; 2]` instead of `(u32, u32)` to represent a range
	/// of text is that I want to emphasize that the `[u32; 2]` is the closed
	/// interval, which means both the start and the end are included in the
	/// range.
...
impl<'text> MagicString<'text> {
	...
	pub fn indent_with(&mut self, opts:IndentOptions) -> &mut Self {
		...
		while let Some(chunk_idx) = next_chunk_id {
			// Make sure the `next_chunk_id` is updated before we split the chunk.
			// Otherwise, we might process the same chunk twice.
